...
def load_send():
    ...
    notify_file = cur_path + "/
            notify.py"
          

    if 
            path.exists(notify_file):
...
class YP:
    def __init__(self, cookie):
          ...
          7:]
            ...
            self.fruit_url
           = '
            https://happy.mail.10086.cn/jsp/cn/garden/'
    ...
    def run(self):
        if 
            ...
            self.get_tasklist(url
           = 'sign_in_3', app_type = 'cloud_app')
            print(f'\n☁️ 云朵大作战')
            ...
            self.get_tasklist(url
           = 'newsign_139mail', app_type = 'email_app')
    ...
    def send_request(self, url, headers=None, cookies=None, data=None, params=None, method='GET', debug=None,
        ...
        for attempt in range(retries):
           ...
           url, params = params, **request_args)
    ...
    def sso(self):
        ...
        sso_payload = {"account": 
           ...
           "toSourceId": "001005"}
        sso_data = 
            self.send_request(sso_url,
           headers = sso_headers, data = sso_payload, method = 'POST').json()
    ...
    def jwt(self):
        ...
        if token is not None:
            ...
            jwt_url = f"
            https://caiyun.feixin.10086.cn:7071/portal/auth/tyrzLogin.action?ssoToken=
          {token}"
            jwt_data = 
            self.send_request(jwt_url,
           headers = 
            self.jwtHeaders,
           method = 'POST').json()
            if jwt_data['code'] != 0:
    ...
    def signin_status(self):
        ...
        check_url = '
            https://caiyun.feixin.10086.cn/market/signin/page/info?client=app'
          
        check_data = 
            self.send_request(check_url,
           headers = 
            self.jwtHeaders,
           cookies = 
            self.cookies).json()
        ...
        if check_data['msg'] == 'success':
            ...
            if today_sign_in:
                ...
                print('❌ 未签到')
                signin_url = '
            https://caiyun.feixin.10086.cn/market/manager/commonMarketconfig/getByMarketRuleName?marketName=sign_in_3'
          
                signin_data = 
            self.send_request(signin_url,
           headers = 
            self.jwtHeaders,
            ...
            self.log_info(signin_data[
          'msg'])
        else:
            print(check_data['msg'])
            
            self.log_info(check_data[
          'msg'])

    # 戳一下
    def click(self):
        url = "
            https://caiyun.feixin.10086.cn/market/signin/task/click?key=task&id=319"
    ...
    def refresh_notetoken(self):
        note_url = '
            http://mnote.caiyun.feixin.10086.cn/noteServer/api/authTokenRefresh.do'
        ...
        try:
            response = 
            self.send_request(note_url,
           headers = note_headers, data = note_payload, method = "POST")
    ...
    def get_tasklist(self, url, app_type):
        url = f'
            https://caiyun.feixin.10086.cn/market/signin/task/taskList?marketname=
          {url}'
        return_data = 
            self.send_request(url,
           headers = 
        ...
        try:
          ...
          'state', '')
           ...
           task_type = 'month', app_type = 'cloud_app')
          ...
          'state', '')
            ...
            self.do_task(task_id,
           task_type = 'day', app_type = 'cloud_app')
          ...
          'state', '')
            ...
            self.do_task(task_id,
           task_type = 'month', app_type = 'email_app')
    ...
    def do_task(self, task_id, task_type, app_type):
        ...
        task_url = f'
            https://caiyun.feixin.10086.cn/market/signin/task/click?key=task&id=
          {task_id}'
        ...
        if app_type == 'cloud_app':
          ...
          ,
                    ...
                    return_data = 
            self.send_request(url
           = note_url, headers = headers, data = payload,
                                                    method = 'POST').json()
                    if return_data is None:
                        return print('出错了')
                    
            self.notebook_id
           = return_data['notebooks'][0]['notebookId']
                    print('开始创建笔记')
    ...
    def create_note(self, headers):
        note_id = 
            self.get_note_id(
          32)  # 获取随机笔记id
        createtime = str(int(round(
            time.time()
           * 1000)))
        
            time.sleep(
          3)
        updatetime = str(int(round(
            time.time()
           * 1000)))
        note_url = '
            http://mnote.caiyun.feixin.10086.cn/noteServer/api/createNote.do'
    ...
    def wxsign(self):
        ...
        url = '
            https://caiyun.feixin.10086.cn/market/playoffic/followSignInfo?isWx=true'
    ...
    def surplus_num(self):
        ...
        draw_info_url = '
            https://caiyun.feixin.10086.cn/market/playoffic/drawInfo'
          
        draw_url = "
            https://caiyun.feixin.10086.cn/market/playoffic/draw"
          

        draw_info_data = 
            self.send_request(draw_info_url,
           headers = 
            self.jwtHeaders).json()
        ...
        if 
          ...
          'msg'))
            ...
            self.log_info(draw_info_data.get(
          'msg'))
    ...
    def fruitLogin(self):
        ...
        if token is not None:
          ...
          {token}&account={
            self.account}
          &targetSourceId=001208&sourceid=1003&enableShare=1'
            headers = {
                'Host': '
            happy.mail.10086.cn'
          ,
          ...
          ,
            ...
            loginInfoData = 
            requests.request(
          "GET", login_info_url, headers = headers)
            treeCookie = 
            loginInfoData.request.headers[
          'Cookie']
          ...
          login/
            ...
            doLoginData = 
            self.send_request(do_login_url,
           headers = 
            self.treeHeaders).json()
          
            if 
            doLoginData.get(
          'result', {}).get('islogin') != 1:
                return print('❌果园登录失败')
            # 去做果园任务
    ...
    def fruitTask(self):
        ...
        if 
          ...
          f'{
            self.fruit_url}
          user/
            clickCartoon.do?cartoonType=widget'
          ,
                                               headers = 
            self.treeHeaders).json()
          ...
          f'{
            self.fruit_url}
          user/
            clickCartoon.do?cartoonType=color'
          ,
                                               headers = 
            self.treeHeaders).json()
        ...
        for task in task_list:
          ...
          'waterNum', 0)
            ...
            task_state = next(
                (
            state.get(
          'taskState', 0) for state in task_state_result if 
            state.get(
          'taskId') == task_id), 0)
    ...
    def do_fruit_task(self, task_name, task_id, water_num):
        ...
        if 
          ...
          'success'):
            ...
            self.fruit_url}
          task/
            givenWater.do?taskId=
          {task_id}'
            get_water_data = 
            self.send_request(get_water_url,
           headers = 
            self.treeHeaders).json()
    ...
    def tree_info(self):
        ...
        treeinfo_data = 
            self.send_request(treeinfo_url,
           headers = 
            self.treeHeaders).json()
        ...
        if not 
          ...
          'msg', '获取果园任务列表失败')
            ...
            collect_water = 
            treeinfo_data.get(
          'result', {}).get('collectWater', 0)
            tree_level = 
            treeinfo_data.get(
          'result', {}).get('treeLevel', 0)
          ...
          "result").get("msg")}')
            ...
            self.fruit_url}
          user/
            watering.do?isFast=0'
          
            if watering_amount > 0:
                for _ in range(watering_amount):
                    watering_data = 
            self.send_request(watering_url,
           headers = 
            self.treeHeaders).json()
    ...
    def cloud_game(self):
        ...
        game_info_data = 
            self.send_request(game_info_url,
           headers = 
            self.jwtHeaders,
           cookies = 
            self.cookies).json()
          
        if game_info_data and 
            game_info_data.get(
          'code', -1) == 0:
            currnum = 
            game_info_data.get(
          'result', {}).get('info', {}).get('curr', 0)
            count = 
            game_info_data.get(
          'result', {}).get('history', {}).get('0', {}).get('count', '')
            rank = 
            game_info_data.get(
          'result', {}).get('history', {}).get('0', {}).get('rank', '')
    ...
    def receive(self):
        receive_url = "
            https://caiyun.feixin.10086.cn/market/signin/page/receive"
          
        prize_url = f"
            https://caiyun.feixin.10086.cn/market/prizeApi/checkPrize/getUserPrizeLogPage?currPage=1&pageSize=15&_=
          {
            self.timestamp}
          "
        receive_data = 
            self.send_request(receive_url,
           headers = 
            self.jwtHeaders,
           cookies = 
            self.cookies).json()
    ...
    def backup_cloud(self):
        backup_url = '
            https://caiyun.feixin.10086.cn/market/backupgift/info'
          
        backup_data = 
            self.send_request(backup_url,
           headers = 
            self.jwtHeaders).json()
        ...
        if state == -1:
            ...
            print('-领取本月连续备份奖励')
            cur_url = '
            https://caiyun.feixin.10086.cn/market/backupgift/receive'
          
            cur_data = 
            self.send_request(cur_url,
           headers = 
            self.jwtHeaders).json()
        ...
        expend_url = '
            https://caiyun.feixin.10086.cn/market/signin/page/taskExpansion'
            # 每月膨胀云朵
        expend_data = 
            self.send_request(expend_url,
           headers = 
            self.jwtHeaders,
           cookies = 
            self.cookies).json()
        ...
        curMonthBackup = 
            expend_data.get(
          'result', {}).get('curMonthBackup', '')  # 本月备份
        preMonthBackup = 
            expend_data.get(
          'result', {}).get('preMonthBackup', '')  # 上月备份
        curMonthBackupTaskAccept = 
            expend_data.get(
          'result', {}).get('curMonthBackupTaskAccept', '')  # 本月是否领取
        nextMonthTaskRecordCount = 
            expend_data.get(
          'result', {}).get('nextMonthTaskRecordCount', '')  # 下月备份云朵
        acceptDate = 
            expend_data.get(
          'result', {}).get('acceptDate', '')  # 月份
        ...
        if preMonthBackup:
            if curMonthBackupTaskAccept:
                ...
                # 领取
                receive_url = f'
            https://caiyun.feixin.10086.cn/market/signin/page/receiveTaskExpansion?acceptDate=
          {acceptDate}'
                receive_data = 
            self.send_request(receive_url,
           headers = 
    ...
    def open_send(self):
        ...
        pushOn = 
            send_data.get(
          'result', {}).get('pushOn', '')  # 0未开启，1开启，2未领取，3已领取
        firstTaskStatus = 
            send_data.get(
          'result', {}).get('firstTaskStatus', '')
        secondTaskStatus = 
            send_data.get(
          'result', {}).get('secondTaskStatus', '')
        onDuaration = 
            send_data.get(
          'result', {}).get('onDuaration', '')  # 开启时间
